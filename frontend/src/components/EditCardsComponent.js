...
import { editCardsAnimation } from "../animations";
...
const EditCardsComponent = ({ question, answer, cardId, match }) => {
	...
	return (
		<motion.div
			variants={editCardsAnimation}
			initial="hidden"
			animate="show"
			exit="exit"
			className="edit-flash-card-container col-md-8 col-10"
		>
			...
			<div id="buttons">
				<div onClick={() => setIsEditing(!isEditing)}>
					{isEditing ? (
						<button className="btn btn-sm btn-outline-warning" onClick={editCard}>
							Save
						</button>
					) : (
						<svg
